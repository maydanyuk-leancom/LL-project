...
class SignInController extends \yii\web\Controller
{
            ...
            if ($user) {
                if ($model->shouldBeActivated()) {
                    Yii::$app->getSession()->setFlash('alert', [
                        'body' => 'Ваш аккаунт был успешно создан. Проверьте ваш e-mail.',
    ...
    public function actionRequestPasswordReset()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                ...
                Yii::$app->getSession()->setFlash('alert', [
                    'body' => 'Извините, мы не можем сбросить пароль для этого e-mail.',
    ...
    public function actionResetPassword($token)
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate() && $model->resetPassword()) {
            Yii::$app->getSession()->setFlash('alert', [
                'body' => 'Новый пароль был сохранён',
                'options' => ['class' => 'alert-success']
            ]);
            return $this->goHome();
        }
    ...
    public function actionResendEmail()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                ...
                Yii::$app->getSession()->setFlash('alert', [
                    'body' => 'Sorry, we are unable to resend activation link for email provided.',
                    'options' => ['class' => 'alert-danger']
                ]);
            }
        }
    ...
    public function successOAuthCallback($client)
    {
        ...
        if (!$user) {
            ...
            if($email === null){
                $email = ArrayHelper::getValue($attributes, ['emails', 0]);
            }
            $user->email = $email;
            ...
            if ($user->save()) {
                ...
                if ($user->email && User::find()->where(['email' => $user->email])->count()) {
                    ...
                    Yii::$app->session->setFlash(
                        ...
                        [
                            'options' => ['class' => 'alert-danger'],
                            'body' => 'Ошибка в процессе OAuth авторизации.'
                        ]
                    );
                }
